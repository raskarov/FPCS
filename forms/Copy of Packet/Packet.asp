...
with rsStudentInfo
	.CursorLocation = 3
		sql = "SELECT s.intStudent_ID, s.szFirst_Name, s.szLast_Name,e.intPercent_Enrolled_Locked, " & _
			...
			"tblInstructor i on e.intSponsor_Teacher_ID = i.intInstructor_ID " & _
							...
							<table ID="Table5" cellspacing='1' cellpadding='2' style='height:100%;'>
								<tr>	
									...
									<td class="TableCell" valign=middle>
										<% if intPercent_Enrolled_Locked <> "" and Ucase(session.Contents("strRole")) <> "ADMIN" then %>
											&nbsp;<% = intPercent_Enrolled_Locked%>% 
										...
										<script language=javascript>
											function jfLock(){
												...
												var URL = "<%=Application.Value("strWebRoot")%>forms/budget/budgetworksheet.asp?";
								...
								<tr>
									...
									<td class="TableCell" valign=middle>
										...
										<input type=button value="edit" onclick="jfChangeSponsor('<%=intStudent_ID%>');" class="btSmallGray">
									</td>
								</tr>
								<tr>
									<td class="TableCell" valign=top title="This is the enrollment level you currently have based on the amount of the plan you've implemented.">
						...
						<table width=95% align=left ID="Table6">
							<tr>
								<td colspan="10">
									...
									<% if (cint(application.Contents("intYear_Locked")) < cint(session.Contents("intSchool_Year")) and _
										ucase(session.Contents("strRole")) = "GUARD") or ucase(session.Contents("strRole")) <>  "GUARD" then
									%>
				...
				<tr >
					<td rowspan=3 colspan=8 class="svplain">
						...
						%>
							...
							must click the 'Save Status & Comments' button in order for the changes to be saved.</li>						
							</ul>
						<%
							end if
						%>
											...
											<td class="TableCell" valign="top" style="width:90%;" align="center">
												<% 
													if ucase(session.Contents("strRole")) = "ADMIN" then
														...
														response.Write "<textarea style='width:99%;' rows='1' wrap='virtual' name='szComments" & rsBudget("intILP_ID") & "' onfocus='this.rows=4;' onblur='this.rows=1;' onKeyDown='jfMaxSize(511,this);' " &_
...
<%
	...
	if rsBudget("intOrdered_Item_ID") & "" <> "" then 
		...
		if rsBudget("bolReimburse")  then
			strItemType = "Reimburse: "
		else
			strItemType = "Requisition: "
		end if
